...
+ (NSString *)getFileDocumentPath:(NSString *)fileName
{
    ...
    NSString *documentDirectory = [ZBPathTool getDocumentPath];
...
+ (NSString *)getFileCachePath:(NSString *)fileName
{
    ...
    NSString *cacheDirectory = [ZBPathTool getCachePath];
...
+ (BOOL)isExistFileInDocument:(NSString *)fileName
{
    ...
    NSString *filePath = [ZBPathTool getFileDocumentPath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
+ (BOOL)isExistFileInCache:(NSString *)fileName
{
    ...
    NSString *filePath = [ZBPathTool getFileCachePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if (kIsEmpty(aFolderNameInDoc))
    {
        return YES ;
    }
    NSString *filePath = [ZBPathTool getFileDocumentPath:aFolderNameInDoc];
    if (nil == filePath)
    {
        return YES;
    }
...
{
    if (kIsEmpty(aFolderNameInCahe))
    {
        return YES ;
    }
    
    if (![ZBPathTool isExistFileInCache:aFolderNameInCahe]) {
        return YES;
    }
    
    NSString *filePath = [ZBPathTool getFileCachePath:aFolderNameInCahe];
    if (nil == filePath)
    {
        return YES;
    }
...
//{
//    NSString *filePath = [ZBPathTool getComicBookDir:bookId];
//    if (nil == filePath)
//    {
//        return YES;
//    }
...
+ (BOOL)isExistFileInResource:(NSString *)fileName
{
    ...
    NSString *filePath = [ZBPathTool getFileResourcePath:fileName];
    if (nil == filePath)
    {
        return NO;
    }
...
{
    if (kIsEmpty(resourceName))
    {
        return NO;
    }
    //获取资源文件的存放目录进行
    NSString *resourcePath = [ZBPathTool getFileResourcePath:resourceName];
    NSString *documentPath = [ZBPathTool getFileDocumentPath:resourceName];
    ...
    if ([ZBPathTool isExistFile:documentPath])
    {
        // 如果文件已存在， 那么先删除原来的
        [ZBPathTool deleteFileAtPath:documentPath];
    }
...
+ (BOOL)createDirectoryAtDocument:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [ZBPathTool getFileDocumentPath:dirName];
...
+ (BOOL)createDirectoryAtCache:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *dirPath = [ZBPathTool getFileCachePath:dirName];
    if ([fileManager fileExistsAtPath:dirPath])
    {
        return YES;
    }
...
+ (BOOL)createDirectoryAtTemporary:(NSString *)dirName
{
    ...
    NSFileManager *fileManager = [NSFileManager defaultManager];
    NSString *tempPath = [ZBPathTool getTemporaryPath];
...
+ (NSString *)reCorrentPathWithPath:(NSString *)path
{
    ...
    NSString *docPath = [ZBPathTool getDocumentPath];
    ...
    if (range.length <= 0)
    {
        ...
        if (docRange.length > 0)
        {
            ...
            NSString *newPath = [ZBPathTool getFileDocumentPath:relPath];
            return newPath;
        }
    }
    return path;
...
{
    return [ZBPathTool getTemporaryPath];
}
